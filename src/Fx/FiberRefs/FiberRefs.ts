...
export function FiberRefs(state: Atomic.Atomic<FiberRefsState>): FiberRefs {
  ...
  const setReference = <R extends FiberRef.AnyFiberRef>(ref: R, value: FiberRef.OutputOf<R>) =>
    pipe(
      state,
      Atomic.update((s) => ({ ...s, references: new Map([...s.references, [ref, value]]) })),
    )
  ...
  const locally: FiberRefs['locally'] = (ref, value) =>
    pipe(
      state,
      Atomic.fork((s) => ({ ...s, references: new Map([...s.references, [ref, value]]) })),
      FiberRefs,
    )
  ...
  const inherit: FiberRefs['inherit'] = Fx.Fx(function* () {
    const { fiberRefs } = yield* Fx.getFiberContext

    yield* Fx.zipAll(Array.from(state.get().references).map(([k, v]) => fiberRefs.join(k, v)))
  })
  ...
  function* initializeFiberRef<R, E, A>(ref: FiberRef.FiberRef<R, E, A>) {
    ...
    if (fibers.has(ref)) {
      return yield* Fx.join(fibers.get(ref)!)
    }

    // Initial accessor to this data
    const fiber = yield* Fx.fork(ref.initial)

    pipe(
      state,
      Atomic.update((s) => ({ ...s, fibers: new Map([...s.fibers, [ref, fiber]]) })),
    )

    const value = yield* Fx.join(fiber)
