...
export function makeRef<R, E, A>(
  ...
  return Effect.contextWith((ctx: Context.Context<R>) => new RefSubjectImpl(Effect.provideContext(effect, ctx), eq))
}
...
class RefSubjectImpl<E, A> extends HoldFx<never, E, A> implements RefSubject<E, A> {
  ...
  constructor(readonly initialize: Effect.Effect<never, E, A>, readonly eq: Equivalence.Equivalence<A>) {
    super(never<E, A>())
  ...
  readonly event: RefSubject<E, A>["event"] = methodWithTrace((trace) =>
    (a: A) => Effect.catchAllCause(this.set(a), this.error).traced(trace)
  )

  readonly end = methodWithTrace((trace) =>
    () => Effect.suspend(() => this.fiber ? Fiber.interrupt(this.fiber) : Effect.unit()).traced(trace)
  )
    ...
    return methodWithTrace((trace) =>
      <R2, E2, B>(
        ...
        Effect.flatMap(
          this.get,
          (a1) =>
            this.lock(Effect.flatMap(f(a1), ([b, a2]) =>
              Effect.suspend(() => {
  ...
  readonly modify: RefSubject<E, A>["modify"] = (f) => this.modifyEffect((a) => Effect.sync(() => f(a)))

  readonly updateEffect: RefSubject<E, A>["updateEffect"] = (f) =>
    this.modifyEffect((a) => Effect.map(f(a), (a) => [a, a]))

  readonly update: RefSubject<E, A>["update"] = (f) => this.updateEffect((a) => Effect.sync(() => f(a)))
  ...
  readonly mapEffect: RefSubject<E, A>["mapEffect"] = <R2, E2, B>(f: (a: A) => Effect.Effect<R2, E2, B>) =>
    new ComputedImpl<never, E, A, R2, E2, B>(this, f)

  readonly map: RefSubject<E, A>["map"] = (f) => this.mapEffect((a) => Effect.sync(() => f(a)))
}

class TupleRefSubjectImpl<S extends ReadonlyArray<RefSubject.Any>> extends HoldFx<
  never,
  Fx.ErrorsOf<S[number]>,
  {
    readonly [K in keyof S]: Fx.OutputOf<S[K]>
  ...
  >
{
  ...
  constructor(readonly subjects: S) {
    super(
      hold(combineAll(...subjects)) as any
    )
...
class StructRefSubjectImpl<S extends RR.ReadonlyRecord<RefSubject.Any>> extends HoldFx<
  never,
  Fx.ErrorsOf<S[number]>,
  {
    readonly [K in keyof S]: Fx.OutputOf<S[K]>
  ...
  >
{
  ...
  constructor(readonly subjects: S) {
    super(
      hold(
        map(combineAll(...Object.entries(subjects).map(([k, s]) => map(s, (x) => [k, x]))), Object.fromEntries)
      ) as any
    )
    ...
    return pipe(
      ...
      Effect.flatMap((a) =>
        this.lock(Effect.gen(function*($) {
          ...
          yield* $(
            Effect.all(
              RR.map(subjects, (s, i) => s.set(a2[i]))
            )
          )
...
class ComputedImpl<R, E, A, R2, E2, B> implements Computed<R | R2, E | E2, B> {
  ...
  ): Computed<R | R2 | R3, E | E2 | E3, C> => new ComputedImpl<R | R2, E | E2, B, R3, E3, C>(this, f)
