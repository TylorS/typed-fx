import { Cause, Effect, MutableRef, Option, Ref } from 'effect'
...
export interface UnsafeEmitter<E, A> {
  ...
  readonly unsafeFailCause: (cause: Cause.Cause<E>) => void
...
export interface HoldSubject<E, A> extends Subject<E, A> {
  readonly get: Effect.Effect<never, never, Option.Option<A>>
}
...
export namespace HoldSubject {
  export const unsafeMake = <E, A>(): HoldSubject<E, A> => {
    ...
    return {
      ...FX_BRANDING,
      get: Effect.sync(() => MutableRef.get(h.value)),
      run: (e) => h.run(e),
      emit: (a) => h.emit(a),
...
//   export const unsafeMake = <A, E = never>(initial: () => A): RefSubject<E, A> => {
//     const h = new Hold<never, E, A>(never)
//     const ref = emitRefChanges(invmapRef(maybeRef, Option.getOrElse(initial), Option.some), h)

//     // Ensure there is always a value in the Ref
//     h.value.set(Option.some(initial()))

//     return {
//       ...FX_BRANDING,
//       [Ref.RefTypeId]: {} as any,
//       get: ref.get,
...
//     pipe(ref.getAndUpdateSome(flow(to, f, Option.map(from))), Effect.map(to))
...
//   const updateSome: Ref.Ref<B>['updateSome'] = (f) => ref.updateSome(flow(to, f, Option.map(from)))

//   const updateSomeAndGet: Ref.Ref<B>['updateSomeAndGet'] = (f) =>
//     pipe(ref.updateSomeAndGet(flow(to, f, Option.map(from))), Effect.map(to))
