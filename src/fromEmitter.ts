...
import { Deferred, Effect, Runtime } from "@typed/fx/externals"
...
export function fromEmitter<E, A, R, E2>(
  ...
  return Fx(<R2>(sink: Sink<R2, E | E2, A>): Effect.Effect<R | R2, never, void> =>
    Effect.scoped(Effect.gen(function*($) {
      ...
      const emitter: Emitter<E, A> = {
        ...
        error: methodWithTrace((trace) =>
          (cause) => run(forkIn(Effect.intoDeferred(sink.error(cause), deferred)).traced(trace))
        ),
        end: methodWithTrace((trace) => () => run(Deferred.succeed(deferred, undefined).traced(trace)))
      }
