...
export const filterMapEffect: {
  ...
  (trace) =>
    <R, E, A, R2, E2, B>(
      self: Fx<R, E, A>,
      f: (a: A) => Effect.Effect<R2, E2, Option.Option<B>>
    ): Fx<R | R2, E | E2, B> => new FilterMapEffectFx(self, f).traced(trace)
)
...
export const filterEffect: {
  ...
  <A, R2, E2>(
    f: (a: A) => Effect.Effect<R2, E2, boolean>
  ): <R, E>(self: Fx<R, E, A>) => Fx<R | R2, E | E2, A>
} = dualWithTrace(
  2,
  (trace) =>
    <R, E, A, R2, E2>(
      ...
      filterMapEffect(self, (a) => Effect.map(f(a), (b) => (b ? Option.some(a) : Option.none()))).traced(trace)
)
...
export const filterNotEffect: {
  ...
  (trace) =>
    <R, E, A, R2, E2>(
      ...
      filterMapEffect(self, (a) => Effect.map(f(a), (b) => (b ? Option.none() : Option.some(a)))).traced(trace)
)

export class FilterMapEffectFx<R, E, A, R2, E2, B> extends BaseFx<
  R | R2,
  E | E2,
  B
> {
