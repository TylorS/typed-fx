...
export const mapAccum: {
  ...
  <A, B, C>(initial: B, f: (acc: B, a: A) => readonly [C, B]): <R, E>(fx: Fx<R, E, A>) => Fx<R, E, C>
} = dualWithTrace(
  3,
  (trace) =>
...
export class MapAccumFx<R, E, A, B, C> extends BaseFx<R, E, C> {
  ...
  constructor(readonly fx: Fx<R, E, A>, readonly initial: B, readonly f: (acc: B, a: A) => readonly [C, B]) {
    super()
  }
...
export class MapAccumSink<E, A, B, C> implements Sink<E, A> {
  ...
  constructor(readonly sink: Sink<E, C>, readonly initial: B, readonly f: (acc: B, a: A) => readonly [C, B]) {}
