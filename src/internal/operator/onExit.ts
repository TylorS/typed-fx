...
export const onExit: {
  ...
  ): <R, A>(self: Fx<R, E, A>) => Fx<R | R2, E, A>
} = dualWithTrace(
  2,
  (trace) =>
    <R, E, A, R2, B>(self: Fx<R, E, A>, f: (exit: Exit.Exit<E, unknown>) => Effect.Effect<R2, never, B>) =>
...
export class OnExitFx<R, E, A, R2, B> extends BaseFx<R | R2, E, A> {
  ...
  constructor(readonly self: Fx<R, E, A>, readonly f: (exit: Exit.Exit<E, unknown>) => Effect.Effect<R2, never, B>) {
    super()
  }
  ...
  run(sink: Sink<E, A>) {
    return Effect.contextWithEffect((ctx: Context.Context<R | R2 | Scope.Scope>) =>
      this.self.run(
        Sink(
          sink.event,
          (cause) => Effect.provideContext(Effect.flatMap(this.f(Exit.failCause(cause)), () => sink.error(cause)), ctx),
          () => Effect.provideContext(Effect.flatMap(this.f(Exit.unit()), () => sink.end()), ctx)
        )
      )
    )
  }
