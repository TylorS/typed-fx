...
export const snapshot: {
  <R, E, A, R2, E2, B, C>(self: Fx<R, E, A>, snapshot: Fx<R2, E2, B>, f: (a: A, b: B) => C): Fx<R | R2, E | E2, C>
  <A, R2, E2, B, C>(snapshot: Fx<R2, E2, B>, f: (a: A, b: B) => C): <R, E>(self: Fx<R, E, A>) => Fx<R | R2, E | E2, C>
} = dualWithTrace(3, (trace) =>
...
> {
  ...
  run(sink: Sink<E | E2, C>) {
    ...
    return Effect.gen(function*($) {
      ...
      yield* $(
        Effect.forkScoped(snapshot.observe((b) => Ref.set(ref, Option.some(b))))
      )
