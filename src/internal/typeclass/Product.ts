...
export const struct: <const R extends Readonly<Record<string, Fx<any, any, any>>>>(
  fields: R
) => Fx<Fx.ResourcesOf<R[string]>, Fx.ErrorsOf<R[string]>, { readonly [K in keyof R]: Fx.OutputOf<R[K]> }> = P.struct(
  Product
) as <
...
export const tuple: <const R extends ReadonlyArray<Fx<any, any, any>>>(
  ...elements: R
) => Fx<Fx.ResourcesOf<R[number]>, Fx.ErrorsOf<R[number]>, { readonly [K in keyof R]: Fx.OutputOf<R[K]> }> = P.tuple(
  Product
) as <
