...
import { map } from "@typed/fx/internal/operator"
...
export const struct: <R extends Readonly<Record<string, Fx<any, any, any>>>>(
  fields: R
) => Fx<Fx.ServicesOf<R[string]>, Fx.ErrorsOf<R[string]>, { readonly [K in keyof R]: Fx.OutputOf<R[K]> }> = P.struct(
  Product
) as <
  R extends Readonly<Record<string, Fx<any, any, any>>>
>(
  fields: R
) => Fx<
...
export const tuple: <R extends ReadonlyArray<Fx<any, any, any>>>(
  ...elements: R
) => Fx<Fx.ServicesOf<R[number]>, Fx.ErrorsOf<R[number]>, { readonly [K in keyof R]: Fx.OutputOf<R[K]> }> = P.tuple(
  Product
) as <
  R extends ReadonlyArray<Fx<any, any, any>>
>(
  ...elements: R
) => Fx<
  Fx.ServicesOf<R[number]>,
