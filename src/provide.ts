...
export function provideContext<R, E, A>(
  ...
  return Fx(<R2>(sink: Sink<R2, E, A>): Effect.Effect<R2, never, void> =>
...
export function provideSomeContext<R, E, A, R2>(
  ...
  return Fx(<R3>(sink: Sink<R3, E, A>): Effect.Effect<Exclude<R, R2> | R3, never, void> =>
    Effect.contramapContext(fx.run(sink), (ctx) => Context.merge(ctx as Context.Context<R | R3>, context))
  )
}
...
export function provideServiceEffect<R, E, A, I, S, R2, E2>(
  ...
  return Fx(<R3>(sink: Sink<R3, E | E2, A>): Effect.Effect<Exclude<R, I> | R2 | R3, never, void> =>
    Effect.matchCauseEffect(
      ...
      (s) =>
        Effect.contramapContext(
          fx.run(sink),
          (ctx) => Context.merge(ctx as Context.Context<R | R2 | R3>, Context.make(tag, s))
        )
    )
  )
}
